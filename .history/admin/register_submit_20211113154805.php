...
if (isset($_POST['btnRegister']) && $_POST["userName"] !='' && $_POST["userEmail"] !='' && $_POST["userPass"] !='' && $_POST["userRePass"] !='' ){
    ...
    if($count_email > 0 || $count_user > 0 ){
        ...
        $query = "INSERT INTO user (user_level,user_username, user_email, user_password ) VALUES ('$level', '$username','$email', '$password')";
        $result = mysqli_query($conn, $query);
        if($result){
            $_SESSION['message'] = 'Sign Up Success . You can login to the system !';
