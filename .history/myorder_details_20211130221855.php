...
if  (isset($_GET['id'])) {
  ...
  $order  = mysqli_query($conn,"SELECT * FROM purchase
    left join user on  purchase.purchase_user_id  = user.user_id
    left join order_prd_id on user.user_id = order_prd_id.user_id 
    left join product on order_prd_id.product_id = product.prd_id 
