...
include('./config/db.php');
    ...
    if (isset($_POST['btnEdit'])) {
        ...
        $query = "UPDATE user set user_img='$update_filename', user_name='$name', user_email='$email', user_address = '$address' , user_contact ='$contact' WHERE user_id=$id";
        $result= mysqli_query($conn, $query);
        if(!$result) {
            die("Query Failed.");
        }
                ...
                <form action="update.php?id=<?php echo $_SESSION["login"][0]?>" method="POST" enctype="multipart/form-data">
                    <div class="update-profile-form">
                        ...
                        <div class="update-profile-form-group">
                            ...
                            <input type="email" class="update-profile-form-input" name="email" value="<?php echo $row['user_email'];?>"/>
                        </div>
                        <div class="update-profile-form-group">
                            <label class="update-profile-form-header">Address</label>
                            <input type="text" class="update-profile-form-input" name="address" value="<?php echo $row['user_address'];?>"/>
                        </div>
                        <div class="update-profile-form-group">
                            <label class="update-profile-form-header">Contact</label>
                            <input type="number" class="update-profile-form-input" name="contact" value="<?php echo $row['user_contact'];?>"/>
                        </div>
                        <div class="update-profile-form-group">
